...
VRSky 是面向 Godot 4 Mobile 渲染器和 OpenXR 的超轻量 24 小时 VR 天空插件，当前主要验证目标为 Meta Quest 3 独立运行与 PC 串流。核心目标是用离线贴图替代实时大气、云和星空计算，只让太阳、月亮、方向光和环境光以低频率更新。

## 纯 Rust 运行时

插件运行时完全由 Rust GDExtension 实现，不包含 GDScript 控制器或后备实现。

`VrSkyController` 原生类直接继承 `Node3D`，负责：
...
`VrSkyController` 同时是编辑器工具类。实例化 `vrsky.tscn` 后可直接在 3D 编辑器中预览天空；修改手动时间、系统时间同步、太阳方位偏移或时间轴路径时会立即刷新预览。
...
- VRSky 使用 Godot 4.6 Mobile 渲染器和 OpenXR；桌面串流预览与 Quest 独立运行都以 Mobile 渲染器为验证基线。

## 目录

- `addons/vrsky/`：Godot 插件、GDExtension 描述文件、Shader、场景和天空资源。
...
构建完成后，将整个 `addons/vrsky` 目录复制到目标 Godot 项目，并实例化：

```text
res://addons/vrsky/vrsky.tscn
```

场景根节点类型为 `VrSkyController`、默认节点名为 `VRSky`。插件只管理天空、太阳光、月光和环境光，不创建相机、`XROrigin3D`、控制器节点或输入映射；这些仍由目标 VR 项目负责。
...
天空资源位于 `addons/vrsky/assets/sky/sky_HHMM.png`，时间轴在 8 个关键时段间连续插值。正式资源应采用左右无缝的 2:1 经纬度栅格贴图。

## 构建测试 APK

在 GitHub 仓库的 **Actions → Build VRSky Quest APK → Run workflow** 手动执行。工作流会先构建 Rust Linux/Android GDExtension，再导出 Quest 3 ARM64 APK，并验证 APK 内原生库。
